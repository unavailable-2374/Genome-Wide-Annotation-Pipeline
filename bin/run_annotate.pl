...
my ($RM_species, $RM_lib, $genome, $out_prefix, $pe1, $pe2, $single_end, $protein, $cpu, $trimmomatic, $strand_specific, $sam2transfrag, $ORF2bestGeneModels, $augustus_species, $HMM_db, $BLASTP_db, $gene_prefix, $cmdString, $enable_augustus_training_iteration, $config, $use_existed_augustus_species, $step, $polish, $help);
GetOptions(
    "help|h" =>\$help,
...
# Step 3: maker_pred
print STDERR "\n============================================\n";
print STDERR "Step 3: maker_pred " . "(" . (localtime) . ")" . "\n";
mkdir "3.maker_pred" unless -e "3.maker_pred";
unless ((-e "3.maker_pred.ok") && $step ne "geta_noly" ){
  chdir "3.maker_pred";
  $pwd = `pwd`; print STDERR "PWD: $pwd";
  $cmdString = "stringtie -p 10 -o tissue.gtf ../2.hisat2/hisat2.sorted.bam";
  system($cmdString) == 0 or die "failed to execute: $cmdString\n";
  $cmdString = "gffread tissue.gtf -g $genome -w transcripts.fa";
  system($cmdString) == 0 or die "failed to execute: $cmdString\n";
  $cmdString = "Launch_PASA_pipeline.pl -c $dirname/config/polish_mRNA.txt -C -R -g $genome --CPU $cpu --ALIGNERS minimap2 -t transcripts.fa > PASA.out 2> PASA.log";
  system($cmdString) == 0 or die "failed to execute: $cmdString\n";
  mkdir "run_maker" unless -e "run_maker";
  chdir "run_maker";
  unless (-e "3.maker_round_1.ok") {
    system("maker -CTL") == 0 or die "failed to execute: maker run failed\n";
    open(CTL,"maker_opts.ctl") or die "cannot open file due to $!\n";
    open(FIG,">new.ctl") or die "cannot open file due to $!\n";
    while (<CTL>) {
      chomp;
      if(/^genome=/){print FIG "genome=../../0.RepeatMasker/genome.masked.fasta\n";}
      elsif(/^est=/){print FIG "est=../pasa.sqlite.assemblies.fasta\n";}
      elsif(/^protein=/){print FIG "protein=".$protein."\n";}
      elsif(/^model_org=/){print FIG "model_org=simple\n";}
      elsif(/^protein2genome=/){print FIG "protein2genome=1\n";}
      elsif(/^est2genome=/){print FIG "est2genome=1\n";}
      else{print FIG $_."\n";}
    }
    close FIG;
    my $base = $gene_prefix."_rnd1";
    $cmdString = "srun --mpi=pmi2 maker -base $base -fix_nucleotides new.ctl maker_bopts.ctl maker_exe.ctl > maker_rnd1.out 2> maker_rnd1.log";
    system($cmdString) == 0 or die "failed to execute: $cmdString\n";
    open OUT, ">", "3.maker_round_1.ok" or die $!; close OUT;
  }
  else{
    $cmdString = "MAKER ROUND ONE";
    print STDERR "CMD(Skipped): $cmdString\n";
  }
  unless (-e "3.train_by_maker.ok") {
    $cmdString = "$dirname/scripts/train_by_maker ../../../../0.RepeatMasker/genome.masked.fasta $gene_prefix > train_rnd1.out 2> train_rnd1.log";
    system($cmdString) == 0 or die "failed to execute: $cmdString\n";
    open OUT, ">", "3.train_by_maker.ok" or die $!; close OUT;
  }else{
    $cmdString = "SNAP && AUGUSTUS trianing.";
    print STDERR "CMD(Skipped): $cmdString\n";
  }
  unless (-e "3.maker_round_2.ok") {
    open(CTL,"new.ctl") or die "cannot open file due to $!\n";
    open(FIG,">maker_opts.ctl") or die "cannot open file due to $!\n";
    while (<CTL>) {
      chomp;
      if(/^snaphmm=/){print FIG "snaphmm=".$gene_prefix."_rnd1.maker.output/snap/".$gene_prefix."_rnd1.hmm\n";}
      elsif(/^augustus_species=/){print FIG "augustus_species=".$gene_prefix."_rnd1\n";}
      else{print FIG $_."\n";}
    }
    close FIG;
    my $base = $gene_prefix."_rnd2";
    $cmdString = "srun --mpi=pmi2 maker -base $base -fix_nucleotides maker_opts.ctl maker_bopts.ctl maker_exe.ctl > maker_rnd2.out 2> maker_rnd2.log";
    system($cmdString) == 0 or die "failed to execute: $cmdString\n";
    open OUT, ">", "3.maker_round_2.ok" or die $!; close OUT;
  }
  else{
    $cmdString = "MAKER ROUND TWO.";
    print STDERR "CMD(Skipped): $cmdString\n";
  }
  chdir "../../";
  open OUT, ">", "3.maker_pred.ok" or die $!; close OUT;
}
else{
  $cmdString = "MAKER prediction.";
  print STDERR "CMD(Skipped): $cmdString\n";
}

# Step 4: Transcript
print STDERR "\n============================================\n";
print STDERR "Step 4: Transcript " . "(" . (localtime) . ")" . "\n";
mkdir "4.transcript" unless -e "4.transcript";
unless (($pe1 && $pe2) or $single_end)  {
    open OUT, ">", "4.transcript.ok" or die $!; close OUT;
    chdir "4.transcript";
    open OUT, ">", "transfrag.genome.gff3" or die $!; close OUT;
    chdir "../";
}
unless ((-e "4.transcript.ok") && $step ne "maker_only") {
    chdir "4.transcript";
    $pwd = `pwd`; print STDERR "PWD: $pwd";
...
=cut
    ...
    print STDERR "Skip Step 4 for the file 4.transcript.ok exists\n";
}

# Step 5: Homolog
print STDERR "\n============================================\n";
print STDERR "Step 5: Homolog " . "(" . (localtime) . ")" . "\n";
mkdir "5.homolog" unless -e "5.homolog";
unless ((-e "5.homolog.ok")&& $step ne "maker_only") {
    chdir "5.homolog";
    $pwd = `pwd`; print STDERR "PWD: $pwd";

    my $max_gene_length = 20000;
    open IN, "../4.transcript/transfrag.genome.gff3" or die "Can not open the file ../4.transcript/transfrag.genome.gff3, $!\n";
    my @gene_length;
    while (<IN>) {
        if (m/\tgene\t(\d+)\t(\d+)\t/) {
    ...
    chdir "../";
    open OUT, ">", "5.homolog.ok" or die $!; close OUT;
}
else {
    print STDERR "Skip Step 5 for the file 5.homolog.ok exists\n";
}

# Step 6: Augustus gene prediction
print STDERR "\n============================================\n";
print STDERR "Step 6: Augustus/HMM Trainning " . "(" . (localtime) . ")" . "\n";
mkdir "6.augustus" unless -e "6.augustus";
unless ((-e "6.augustus.ok")&& $step ne "maker_only") {
    chdir "6.augustus";
    $pwd = `pwd`; print STDERR "PWD: $pwd";
    ...
    if ($use_existed_augustus_species) {
        ...
        close OUT;
        $cmdString = "$dirname/bin/paraCombineGeneModels --cpu $cpu $config{'paraCombineGeneModels'} blank.augustus.gff3 ../../4.transcript/transfrag.genome.gff3 ../../5.homolog/genewise.gff3 blank.intron.gff";
        print STDERR (localtime) . ": CMD: $cmdString\n";
        system("$cmdString") == 0 or die "failed to execute: $cmdString\n";
    ...
    $cmdString = "$dirname/bin/prepareAugusutusHints $config{'prepareAugusutusHints'} bam2intronHints.gff ../4.transcript/transfrag.genome.gff3 ../5.homolog/genewise.gff3 ../5.homolog/genewise.start_stop_hints.gff > hints.gff";
    ...
    chdir "../";
    open OUT, ">", "6.augustus.ok" or die $!; close OUT;
}
else {
    print STDERR "Skip Step 6 for the file 6.augustus.ok exists\n";
}


# Step 7: CombineGeneModels
print STDERR "\n============================================\n";
print STDERR "Step 7: CombineGeneModels " . "(" . (localtime) . ")" . "\n";
mkdir "7.combineGeneModels" unless -e "7.combineGeneModels";
unless ((-e "7.combineGeneModels.ok")&& $step ne "maker_only") {
    chdir "7.combineGeneModels";
    $pwd = `pwd`; print STDERR "PWD: $pwd";
...
geneModels.i.coding.gff3\t对geneModels.h.coding.gff3中的基因模型进行了强制补齐\n";
    ...
    my $cmdString1 = "$dirname/bin/paraCombineGeneModels $config{'paraCombineGeneModels'} --cpu $cpu ../6.augustus/augustus.gff3 ../4.transcript/transfrag.genome.gff3 ../5.homolog/genewise.gff3 ../6.augustus/hints.gff &> /dev/null";
    ...
    my $cmdString1 = "perl -p -e 's/(=[^;]+)\.t1/\$1.t01/g;' ../6.augustus/training/geneModels.gff3 > geneModels.c.gff3;";
    ...
    $cmdString1 = "$dirname/bin/alternative_splicing_analysis $config{'alternative_splicing_analysis'} geneModels.b.gff3 ../4.transcript/intron.txt ../4.transcript/base_depth.txt > geneModels.gb_AS.gff3 2> alternative_splicing.gb.stats; $dirname/bin/GFF3_add_CDS_for_transcript $genome geneModels.gb_AS.gff3 > geneModels.gb.gff3";
    $cmdString2 = "$dirname/bin/alternative_splicing_analysis $config{'alternative_splicing_analysis'} geneModels.e.gff3 ../4.transcript/intron.txt ../4.transcript/base_depth.txt > geneModels.ge_AS.gff3 2> alternative_splicing.ge.stats; $dirname/bin/GFF3_add_CDS_for_transcript $genome geneModels.ge_AS.gff3 > geneModels.ge.gff3";
    $cmdString3 = "$dirname/bin/alternative_splicing_analysis $config{'alternative_splicing_analysis'} geneModels.f.gff3 ../4.transcript/intron.txt ../4.transcript/base_depth.txt > geneModels.gf_AS.gff3 2> alternative_splicing.gf.stats; $dirname/bin/GFF3_add_CDS_for_transcript $genome geneModels.gf_AS.gff3 > geneModels.gf.gff3";
    ...
    chdir "../";
    open OUT, ">", "7.combineGeneModels.ok" or die $!; close OUT;
}
else {
    print STDERR "Skip Step 6 for the file 7.combineGeneModels.ok exists\n";
}

# Step 7: GETA OutPut
print STDERR "\n============================================\n";
print STDERR "Step 7:GETA OutPut " . "(" . (localtime) . ")" . "\n";
unless ($step ne "maker_only"){
    chdir "../";
    $pwd = `pwd`; print STDERR "PWD: $pwd";

    # 7.1 输出GFF3格式文件基因结构注释信息
    $cmdString = "$dirname/bin/GFF3Clear --GFF3_source GETA --gene_prefix $gene_prefix --gene_code_length 6 --genome $genome $out_prefix.tmp/7.combineGeneModels/geneModels.i.coding.gff3 > $out_prefix.GeneModels.gff3 2> /dev/null";
    print STDERR (localtime) . ": CMD: $cmdString\n";
    system("$cmdString") == 0 or die "failed to execute: $cmdString\n";

    $cmdString = "$dirname/bin/GFF3Clear --GFF3_source GETA --gene_prefix ${out_prefix}ncGene  --gene_code_length 6 --genome $genome --no_attr_add $out_prefix.tmp/7.combineGeneModels/geneModels.h.lncRNA.gff3 > $out_prefix.GeneModels_lncRNA.gff3 2> /dev/null";
    print STDERR (localtime) . ": CMD: $cmdString\n";
    system("$cmdString") == 0 or die "failed to execute: $cmdString\n";

    $cmdString = "$dirname/bin/GFF3Clear --GFF3_source GETA --gene_prefix ${out_prefix}lqGene --gene_code_length 6 --genome $genome --no_attr_add --coverage 0.6 $out_prefix.tmp/7.combineGeneModels/geneModels.h.lowQuality.gff3 > $out_prefix.GeneModels_lowQuality.gff3 2> /dev/null";
    print STDERR (localtime) . ": CMD: $cmdString\n";
    system("$cmdString") == 0 or die "failed to execute: $cmdString\n";
    ...
    if (($pe1 && $pe2) or $single_end) {
        open IN, "$out_prefix.tmp/4.transcript/transfrag.genome.gff3" or die "Can not open file $out_prefix.tmp/4.transcript/transfrag.genome.gff3, $!";
    ...
    if ($protein) {
        open IN, "$out_prefix.tmp/5.homolog/genewise.gff3" or die "Can not open file $out_prefix.tmp/5.homolog/genewise.gff3, $!";
    ...
    if (-e "$out_prefix.tmp/6.augustus/augustus.2.gff3") {
        open IN, "$out_prefix.tmp/6.augustus/training_again/secondtest.out" or die "Can not open file $out_prefix.tmp/6.augustus/training_again/secondtest.out, $!";
    }
    else {
        open IN, "$out_prefix.tmp/6.augustus/training/secondtest.out" or die "Can not open file $out_prefix.tmp/6.augustus/training/secondtest.out, $!";
    }
    ...
    open IN, "$out_prefix.tmp/6.augustus/augustus.gff3" or die "Can not open file $out_prefix.tmp/6.augustus/augustus.gff3, $!";
    while (<IN>) {
    ...
    open IN, "$out_prefix.tmp/7.combineGeneModels/geneModels.a.gff3" or die "Can not open file $out_prefix.tmp/7.combineGeneModels/geneModels.a.gff3, $!";
    my ($num_of_gene_a, $num_of_gene_b) = (0, 0);
    while (<IN>) {
        $num_of_gene_a ++ if m/\tgene\t/;
    }
    close IN;
    open IN, "$out_prefix.tmp/7.combineGeneModels/geneModels.b.gff3" or die "Can not open file $out_prefix.tmp/7.combineGeneModels/geneModels.b.gff3, $!";
    while (<IN>) {
    ...
    open IN, "$out_prefix.tmp/7.combineGeneModels/picked_evidence_geneModels.log" or die "Can not open file $out_prefix.tmp/7.combineGeneModels/picked_evidence_geneModels.log, $!";
    ...
    open IN, "$out_prefix.tmp/7.combineGeneModels/fillingEndsOfGeneModels.1.log" or die "Can not open file $out_prefix.tmp/7.combineGeneModels/fillingEndsOfGeneModels.1.log, $!";
    my @line = <IN>;
    close IN;
    ...
    open IN, "$out_prefix.tmp/7.combineGeneModels/get_valid_geneModels.log" or die "Can not open file $out_prefix.tmp/7.combineGeneModels/get_valid_geneModels.log, $!";
    $_ = <IN>; 
    close IN;
    my @number = m/(\d+)/g;
...
# Step 8: combine_Maker_GETA
print STDERR "\n============================================\n";
print STDERR "Step 7: combine_Maker_GETA " . "(" . (localtime) . ")" . "\n";
mkdir "7.Polcombine_Maker_GETAishing" unless -e "7.combine_Maker_GETA";
unless ($step ne "geta_only" && $step ne "maker_only") {
  chdir "7.combine_Maker_GETA";
  $pwd = `pwd`; print STDERR "PWD: $pwd";
  my $dir = $gene_prefix."_rnd2";
  open(MGF,"../3.maker_pred/run_maker/$dir./$dir.all.maker.gff") or die "cannot open file due to $!\n";
  open(OUT,">maker_gene.gff3") or die "cannot open file due to $!\n";
  open(LIST,">maker_gene.list") or die "cannot open file due to $!\n";
  while(<MGF>){
    chomp;
    my @arr = split(/\t/);
    if($arr[1] eq "maker"){
      print OUT $_ ."\n";
      if($arr[2] eq "gene"){
        print LIST $arr[0]."\t".$arr[3]."\t".$arr[4]."\t".$arr[8]."\n";
      }
    }
  }close OUT;close LIST;
  open(GFF,"out.GeneModels.gff3") or die "cannot open file due to $!\n";
  open(OUT,">geta.list") or die "cannot open file due to $!\n";
  while(<GFF>){
    chomp;
    my @arr = split(/\t/);
    if($arr[2] eq "gene"){
      print OUT $arr[0]."\t".$arr[3]."\t".$arr[4]."\t".$arr[8]."\n";
    }
  }close OUT;close GFF;
  $cmdString = "bedtools intersect -a maker_gene.list -b geta.list -u > overlap.txt";
  system($cmdString) == 0 or die "failed to execute: $cmdString\n";
  open(IN,"overlap.txt") or die "cannot open file due to $!\n";
  open(OUT,">OV_ID.list") or die "cannot open file due to $!\n";
  my %overlap;
  while(<IN>){
    chomp;
    my @arr = split(/\t/);
  ...
  $cmdString = "grep -v -f OV_ID.list maker_gene.gff3 > out.maker_extra.gff3";
  system($cmdString) == 0 or die "failed to execute: $cmdString\n";
  $cmdString = "cat out.maker_extra.gff3 out.GeneModels.gff3 > merged.gff3";
  system($cmdString) == 0 or die "failed to execute: $cmdString\n";
  open(GFF,"merged.gff3") or die "cannot open file due to $!\n";
  open(OUT,">maker_gene.list") or die "cannot open file due to $!\n";
  while(<GFF>){
    chomp;
    my @arr = split(/\t/);
    if($arr[2] eq "gene"){
  ...
  open(GFF,"out.GeneModels_lowQuality.gff3") or die "cannot open file due to $!\n";
  open(OUT,">geta.list") or die "cannot open file due to $!\n";
  while(<GFF>){
    chomp;
    my @arr = split(/\t/);
  ...
  $cmdString = "bedtools intersect -a geta.list -b maker_gene.list -u > overlap.txt";
  system($cmdString) == 0 or die "failed to execute: $cmdString\n";
  open(IN,"overlap.txt") or die "cannot open file due to $!\n";
  open(OUT,">OV_ID.list") or die "cannot open file due to $!\n";
  my %overlap;
  ...
  $cmdString = "grep -v -f OV_ID.list out.GeneModels_lowQuality.gff3 >> merged.gff3";
  system($cmdString) == 0 or die "failed to execute: $cmdString\n";
  
}

print STDERR "\n============================================\n";
print STDERR "GWAP complete successfully! " . "(" . (localtime) . ")" . "\n\n";
...
sub detecting_dependent_softwares {
    ...
    # 检测maker
    $software_info = `maker -h`;
    if ($software_info =~ m/MAKER/) {
        print STDERR "maker:\tOK\n";
    }
    else {
        die "maker:\tFailed\n\n";
    }
    ...
    # 检测bedtools
    $software_info = `bedtools -h`;
    if ($software_info =~ m/Usage/) {
        print STDERR "bedtools:\tOK\n";
    }
    else {
        die ":bedtools\tFailed\n\n";
    }
