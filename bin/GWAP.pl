...
[General]
    ...
    --work_dir <string>    Required the path 
...
my ($RM_species, $RM_lib, $genome, $out_prefix, $dirs, $pe1, $pe2, $single_end, $protein, $cpu, $trimmomatic, $strand_specific, $sam2transfrag, $ORF2bestGeneModels, $augustus_species, $HMM_db, $BLASTP_db, $gene_prefix, $cmdString, $enable_augustus_training_iteration, $config, $use_existed_augustus_species, $help);
GetOptions(
    "help|h" =>\$help,
...
unless (-e "2.hisat2.ok") {
    ...
    unless (-e "hisat2.sort.ok") {
        print STDERR (localtime) . ": CMD: $cmdString\n";
        system("$cmdString") == 0 or die "failed to execute: $cmdString\n";
        $cmdString = "rm hisat2.sam";
        system("$cmdString") == 0 or die "failed to execute: $cmdString\n";
    }else{print STDERR "CMD(Skipped): $cmdString\n";}
...
unless (-e "3.maker_pred.ok"){
    chdir "3.maker_pred";
    $pwd = `pwd`; print STDERR "PWD: $pwd";
    unless (-e "prepare_for_PASA.ok"){
    ...
    print STDERR (localtime) . ": CMD: $cmdString\n";
    system($cmdString) == 0 or die "failed to execute: $cmdString\n";
    $cmdString = "touch prepare_for_PASA.ok";
    system($cmdString) == 0 or die "failed to execute: $cmdString\n";
    }else{print STDERR "CMD(Skipped): $cmdString\n";}
    unless (-e "PASA.ok"){
    ...
    while (<CTL>) {
        ...
        elsif(/^keep_preds=/){print FIG "keep_preds=1\n";}
        else{print FIG $_."\n";}
    }
    close FIG;
    unless (-e "maker.ok"){
    $cmdString = "srun --mpi=pmi2 maker -base $gene_prefix -fix_nucleotides new.ctl maker_bopts.ctl maker_exe.ctl > maker_rnd1.out 2> maker_rnd1.log";
    print STDERR (localtime) . ": CMD: $cmdString\n";
    system($cmdString) == 0 or die "failed to execute: $cmdString\n";
    $cmdString = "touch maker.ok";
    system($cmdString) == 0 or die "failed to execute: $cmdString\n";
    }else{print STDERR "CMD(Skipped): $cmdString\n";}
    my $dirs = $gene_prefix."_master_datastore_index.log";
    $cmdString = "gff3_merge -s -d $gene_prefix.maker.output/$dirs > $gene_prefix.all.maker.gff";
    print STDERR (localtime) . ": CMD: $cmdString\n";
    system($cmdString) == 0 or die "failed to execute: $cmdString\n";
    open(MGF,"$gene_prefix.all.maker.gff") or die "cannot open file due to $!\n";
...
unless (-e "4.transcript.ok") {
    ...
    $cmdString = "rm ../2.hisat2/hisat2.sorted.sam";
    system("$cmdString") == 0 or die "failed to execute: $cmdString\n";
    ...
    $cmdString = "$dirname/bin/GFF3Clear --GFF3_source GWAP --genome $genome --gene_prefix transfrag --no_attr_add transdecoder2ORF.gff3 > transfrag.genome.gff3 2> /dev/null";
...
unless (-e "5.homolog.ok") {
    ...
    $cmdString = "$dirname/bin/GFF3Clear --GFF3_source GWAP --genome $genome --gene_prefix genewise --no_attr_add genewise.gff3 > out.gff3 2> /dev/null";
    print STDERR (localtime) . ": CMD: $cmdString\n";
    system("$cmdString") == 0 or die "failed to execute: $cmdString\n";
...
unless (-e "6.augustus.ok") {
    ...
    if ($use_existed_augustus_species) {
        ...
        close OUT;
        $cmdString = "$dirname/bin/paraCombineGeneModels_au --cpu $cpu $config{'paraCombineGeneModels'} blank.augustus.gff3 ../../4.transcript/transfrag.genome.gff3 ../../5.homolog/genewise.gff3 ../../3.maker_pred/maker_gene.gff3 blank.intron.gff";
        print STDERR (localtime) . ": CMD: $cmdString\n";
        system("$cmdString") == 0 or die "failed to execute: $cmdString\n";
    ...
    unless (-e "training.ok") {
        ...
        open OUT, ">", "blank.intron.gff" or die "Can not create file blank.intron.gff, $!\n";
        close OUT;
        $cmdString = "$dirname/bin/paraCombineGeneModels --cpu $cpu $config{'paraCombineGeneModels'} blank.augustus.gff3 ../../4.transcript/transfrag.genome.gff3 ../../5.homolog/genewise.gff3 ../../3.maker_pred/maker_gene.gff3 blank.intron.gff";
        print STDERR (localtime) . ": CMD: $cmdString\n";
        system("$cmdString") == 0 or die "failed to execute: $cmdString\n";
    ...
    $cmdString = "rm ../2.hisat2/hisat2.sorted.bam";
    system("$cmdString") == 0 or die "failed to execute: $cmdString\n";
...
geneModels.i.coding.gff3\t对geneModels.h.coding.gff3中的基因模型进行了强制补齐\n";
    ...
    my $cmdString1 = "$dirname/bin/paraCombineGeneModels $config{'paraCombineGeneModels'} --cpu $cpu ../6.augustus/augustus.gff3 ../4.transcript/transfrag.genome.gff3 ../5.homolog/genewise.gff3 ../3.maker_pred/maker_gene.gff3 ../6.augustus/hints.gff &> /dev/null";
    ...
    print STDERR "Skip Step 7 for the file 7.combineGeneModels.ok exists\n";
}

# Step 8: GWAP OutPut
print STDERR "\n============================================\n";
print STDERR "Step 8:GWAP OutPut " . "(" . (localtime) . ")" . "\n";
    chdir "../";
    $pwd = `pwd`; print STDERR "PWD: $pwd";

    # 7.1 输出GFF3格式文件基因结构注释信息
    $cmdString = "$dirname/bin/GFF3Clear --GFF3_source GWAP --gene_prefix $gene_prefix --gene_code_length 6 --genome $genome $out_prefix.tmp/7.combineGeneModels/geneModels.i.coding.gff3 > $out_prefix.GeneModels.gff3 2> /dev/null";
    print STDERR (localtime) . ": CMD: $cmdString\n";
    system("$cmdString") == 0 or die "failed to execute: $cmdString\n";

    $cmdString = "$dirname/bin/GFF3Clear --GFF3_source GWAP --gene_prefix ${out_prefix}ncGene  --gene_code_length 6 --genome $genome --no_attr_add $out_prefix.tmp/7.combineGeneModels/geneModels.h.lncRNA.gff3 > $out_prefix.GeneModels_lncRNA.gff3 2> /dev/null";
    print STDERR (localtime) . ": CMD: $cmdString\n";
    system("$cmdString") == 0 or die "failed to execute: $cmdString\n";

    $cmdString = "$dirname/bin/GFF3Clear --GFF3_source GWAP --gene_prefix ${out_prefix}lqGene --gene_code_length 6 --genome $genome --no_attr_add --coverage 0.6 $out_prefix.tmp/7.combineGeneModels/geneModels.h.lowQuality.gff3 > $out_prefix.GeneModels_lowQuality.gff3 2> /dev/null";
    print STDERR (localtime) . ": CMD: $cmdString\n";
    system("$cmdString") == 0 or die "failed to execute: $cmdString\n";
